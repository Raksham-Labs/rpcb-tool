...
def load_rules(project=None):
    """Built-ins merged with project overrides. Returns a list of rule dicts."""
    merged = {}
    order = []
    ...
    if cfg_path:
        for rule in (_load_yaml(cfg_path).get('rules') or []):
            rid = rule.get('id')
            if not rid:
                raise RulesError(f'rule without an id in {cfg_path}')
            if rid in merged:
                merged[rid].update(rule)          # override / tweak
            else:
    ...
    for rid in order:
        ...
        if 'check' not in rule:
            raise RulesError(f'rule {rid} has no `check` kind')
